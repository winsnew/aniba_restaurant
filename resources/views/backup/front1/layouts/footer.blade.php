<footer class="page-footer text-start bg-brown text-white pt-5 pb-4">
  <div class="container">
    <div class="row">
        <div class="col-3 col-lg-3 col-sm-3">
            <h6><b>CONTACT US</b></h6>
            <p class="p-0 m-0">hello@aniba.com.sg</p>
            <p class="p-0 m-0">+65 9876 4578</p>
            <p class="py-3"> 6 Battery Road, #05</p>
        </div>
        <div class="col-2 col-lg-2 col-sm-2">
            <h6><b>SOCIAL</b></h6>
            <img class="mx-1" src="{{asset('images/instagram.png')}}" width="30" alt="">
            <img class="mx-1" src="{{asset('images/facebook.png')}}" width="30" alt="">
        </div>
        <div class="col-3 col-lg-3 col-sm-3">
            <h6><b>OPENING HOURS</b></h6>
            <div class="py-2">
                <p class="p-0 m-0"> Tuesday - Friday 12PM to 11PM</p>
                <p class="p-0 m-0"> Saturday: 8PM till late</p>
                <p class="p-0 m-0"> Sunday: 11AM to 5PM</p>
            </div>
            <p class="py-2">Closed on Moday</p>
            
        </div>
        <div class="col-4 col-lg-4 col-sm-4">
            <h6><b>JOIN OUR MAILING LIST:</b></h6>
            <div class="row py-3">
                <div class="col-8 col-lg-8 col-sm-8">
                    <input type="text" class="form-control text-white rounded-0 border-0 border-bottom bg-transparent" placeholder="EMAIL">
                    {{-- <p>EMAIL</p>
                    <img class="pt-4" src="{{asset('images/line.png')}}" width="100%" alt="">             --}}
                </div>
                <div class="col-4 col-lg-4 col-sm-4">
                    <button class="text-white border-white py-2 px-4 bg-transparent rounded-pill btn-subscribe" style="font-weight: 700;font-size: 12px">SUBSCRIBE</button>
                </div>
            </div>

            <p> By Subscribing, you agree to receive marketing emails & updates by the FORAGERS group</p>
        </div>
    </div>
    <div class="row">
        <div class="col-6 col-lg-6 col-sm-6">
            <div class="pt-5">
                <img src="{{asset('images/aniba.png')}}" width="60px" alt="">            
                <span class="ps-2">copyright 2022. Privacy Policy</span>
            </div>
        </div>
        <div class="col-6 col-lg-6 col-sm-6">
            <div class="text-end">
                <img src="{{asset('images/symbol.png')}}" width="85px" alt="">            
            </div>
        </div>
    </div>
  </div>
</footer>